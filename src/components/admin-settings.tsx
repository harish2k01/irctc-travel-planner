...
type Settings = {
  ...
  bookingWindowDays: number; bookingOpenHour: number; bookingOpenMinute: number; pnrAutoSyncEnabled: boolean; pnrSyncIntervalMinutes: number;
...
export function AdminSettings({ currentUserId, initialSettings, initialUsers }: { currentUserId: string; initialSettings: Settings; initialUsers: ManagedUser[] }) {
  ...
  async function save(formData: FormData) {
    ...
    try {
      ...
      const payload = {
        ...settings,
        bookingWindowDays: Number(formData.get("bookingWindowDays")), bookingOpenHour: Number(formData.get("bookingOpenHour")), bookingOpenMinute: Number(formData.get("bookingOpenMinute")), pnrSyncIntervalMinutes: Number(formData.get("pnrSyncIntervalMinutes")),
  ...
  return <div className="grid gap-3">
    ...
    <div className="grid gap-3 xl:grid-cols-2">
      <Section title="Access"><SettingRow label="Allow public signups" detail="New visitors can create their own account."><Switch label="Allow public signups" checked={settings.allowSignups} onChange={(value) => setSettings({ ...settings, allowSignups: value })} /></SettingRow></Section>
